package com.adolesce.server.javabasic.jdk8Speciality;
 ...
 *      3). 惰性求值，流在中间处理过程中，只是对操作进行了记录，并不会立即执行，需要等到执行【终止操作】的时候才会进行实际的计算。
...
public class StreamAndOptionalTest {
    ...
    public void testStreamOperation1() {
        ...
        //6、sorted 返回一个新流，流中的元素按照自然排序进行排序 sorted(Comparator comp) 返回一个新流，并且Comparable指定的排序方式进行排序
        ...
        strs.stream().peek(str -> str = str.toUpperCase()).collect(Collectors.toList());
    ...
    public void testStreamOperation2() {
        ...
        myUsers.sort(Comparator.comparing(User::getAge).reversed());
        myUsers.forEach(System.out::println);
        System.out.println("--------------------------------");

        myUsers.stream().sorted(Comparator.comparing(User::getAge));
    ...
    /**
     *  关于map聚合操作对集合中元素影响的测试
     *  结论：
     *      1、管道操作确实不会影响原集合中的元素，比如limit、filter、skip、sorted等等操作，只会在管道中数据做处理，
     *         通常是在原集合基础上做容器性质的处理，最后也是把做完处理的数据放入另一个集合，并不会影响原集合
     *      2、对于集合中的引用数据类型，如果管道中操作了对象中的属性，这个是会造成影响的，因为管道中确实拿着对象的引用去操作了其中的属性
     *      3、对于最终的toList操作，返回的是经过管道处理后的原对象引用集合
     *      4、如果操作的list中装的是Integer、String等类型的引用类型数据，如果对其中元素做了修改，是不会影响原集合的，因为这些元素修改操作
     *         实际上是修改了引用，并没有影响到原来的集合
     */
    @Test
    public void mytest1(){
    ...
    public void testGroup() {
        ...
        //按给定值进行分组（如：按姓名分组）
        Map<Integer, List<User>> kv = users.stream().collect(Collectors.groupingBy(User::getAge));
