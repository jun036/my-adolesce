...
public class TestOomTooManyObject {
    public static void main(String[] args) {
        ...
        System.out.println("product对象占用字节数："+a);
        // 一个字符串占用内存
        ...
        System.out.println("name属性占用字节数："+b);
        String desc = "【全金属全面屏】学生商务办公，全新11代处理器，MX450独显，100%sRGB高色域，指纹识别，快充（更多好货）";
        long c = ClassLayout.parseInstance(desc).instanceSize();
        System.out.println("desc属性占用字节数："+c);
        System.out.println("name字符串数组所占字节数：" + 16 + name.getBytes(StandardCharsets.UTF_8).length);
        System.out.println("desc字符串数组所占字节数：" + 16 + desc.getBytes(StandardCharsets.UTF_8).length);
        // 一个对象估算的内存
